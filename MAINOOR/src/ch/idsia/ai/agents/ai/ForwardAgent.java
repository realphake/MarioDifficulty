...
public class ForwardAgent extends BasicAIAgent implements Agent
{
    ...
    private boolean DangerOfCoin(byte[][] levelScene)
    {
    for (int x = 0; x < levelScene[x].length-1; ++x)
        {
            boolean f = true;
            for(int y = 0; y < levelScene[y].length-1; ++y)
            {
                if(levelScene[x][y] == 34)

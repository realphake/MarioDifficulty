...
public class ArchLevel extends Level{
	...
	public ArchLevel(paramsPCG m)
	{
        ...
        public int getCustomRewards(String type, GamePlay Observations)
        { 
            //System.out.println("C:" + COINS);
            //System.out.println("MC:" + MAX_COINS);
            
            
            int reward_label = 0;
            if("coin".equals(type))
                reward_label = Math.round(((float)COINS/(float)MAX_COINS)*4 +1);
            if("enemy".equals(type))
                reward_label = Math.round(((float)ENEMIES/(float)MAX_ENEMIES)*4 +1);
            if("time".equals(type))
                reward_label = Math.round(((float)Observations.completionTime / 6)*4 +1);
            if("jump".equals(type))
                reward_label = Math.round(((float)this.gaps/MAX_GAPS)*4 +1);
            if("bullet".equals(type))
                reward_label = Math.round(((float)cannons/2)*4 +1);
            if("turtle".equals(type))
                reward_label = Math.round(((float)(Observations.RedTurtlesKilled+Observations.GreenTurtlesKilled)/turtles)*4 +1);
            if("stoner".equals(type))
                reward_label = Math.round(((float)(Observations.timesSwichingPower)/Observations.totalpowerBlocks)*4 +1);
            if("blocks".equals(type))
                reward_label = Math.round((float)(Observations.emptyBlocksDestroyed+Observations.coinBlocksDestroyed+Observations.powerBlocksDestroyed)/(BLOCKS_COINS+BLOCKS_EMPTY+BLOCKS_POWER)*4 +1);
            if("fire".equals(type))
                reward_label = Math.round(((float)(Observations.enemyKillByFire)/MAX_ENEMIES)*4 +1);
            if("goomba".equals(type))
                reward_label = Math.round(((float)(Observations.GoombasKilled)/MAX_ENEMIES)*4 +1);
	...
	public void createArchLevel(paramsPCG m)
        {
            //input the new valuesintothe odds list
               // double sum = m.ODDS_STRAIGHT + m.ODDS_HILL_STRAIGHT + m.ODDS_JUMP + m.ODDS_TUBES + m.ODDS_CANNONS;

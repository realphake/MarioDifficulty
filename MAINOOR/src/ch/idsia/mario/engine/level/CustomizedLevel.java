...
import ch.idsia.mario.engine.LevelFactory;
...
public class CustomizedLevel extends Level implements LevelInterface {
   ...
   // private GamePlay playerM;
    ...
    public void creat(long seed, int difficulty, int type, ARCH_MESSAGE m) {
        ...
        //Get level attributes from Architect.
    ...
    private int buildZone(int x, int maxLength) {
        ...
        int type = 0;
        double A = odds[0];
        double B = odds[0]+odds[1];
        double C = odds[0]+odds[1] + odds[2];
        double D = odds[0]+odds[1] + odds[2] + odds[3];
        
        
        if(t <= A ) type = ODDS_STRAIGHT;
        else if ( t>A && t <= B ) type = ODDS_HILL_STRAIGHT;
        else if ( t>B && t <= C ) type = ODDS_TUBES;
        else if ( t>C && t <= D ) type = ODDS_JUMP;
        else if ( t>D && t <= 100 ) type = ODDS_CANNONS;

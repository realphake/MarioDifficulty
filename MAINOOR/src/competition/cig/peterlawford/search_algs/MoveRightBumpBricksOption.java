...
public class MoveRightBumpBricksOption extends MoveRightOption {//Option<MoveRightBumpBricksOption> {
//	final Frame f;
...
//		// Option 4
//		double base_score = f.mario.getX() - f_old.mario.getX();
//		double height_penalty = Math.abs(f.mario.getY() - f_old.mario.getY())/10;
//		int brick_bonus = f.mario.nBlocksHit - f_old.mario.nBlocksHit;
//		
//		return base_score - height_penalty + brick_bonus;
//	}

	
	
	// This is now inherited from MoveRightOption
...
//	public MoveRightBumpBricksOption(MoveRightBumpBricksOption old, Frame f, int coded_action,
...
//		// Third method
//		double new_dist = 0;
//		MoveRightBumpBricksOption opt_t = this;
//		for (int i=0; i<32; i++) {
//			if (opt_t == null) break;
...
//	}
	...
	public MoveRightBumpBricksOption(MoveRightBumpBricksOption old, Frame f, int coded_action, BlockEvaluator block_eval) {
		super(old, f, coded_action);
//		this.f = f;
//		this.parent = old;
		old.children[coded_action] = this;
//		this.coded_action = coded_action;
		...
		while ((opt_t.parent != null) && (nCycles < 32)) {
			...
			opt_t = (MoveRightBumpBricksOption)opt_t.parent;
		}
...
/*		{
	...
	public MoveRightBumpBricksOption(Frame f, int nCycleCnt) {
		super(null, f, -1);
//		this.f = f;
...
//	}
	...
	public MoveRightBumpBricksOption clone_and_guess(int i,
			BlockEvaluator block_eval) {
		Frame f_new = new Frame(f, false);
		return new MoveRightBumpBricksOption(this, f_new, i, block_eval);		
	}
...
//			System.out.print(delta+" ");
	...
	private double estimate_distance_travelled(
		...
		boolean excited = block_eval.isExcitedAt(mario.x, mario.y);
		if (excited) {
			if ((mario.onGround) && j) xa += 0.5;
		} else {
	...
	private double verify_distance_travelled(
		...
		double result = fudge_distance_travelled2(block_eval, mario_new.x - mario_old.x,
				mario_old.x);
		if (mario_new.nBlocksHit > mario_old.nBlocksHit) result += 1;
		return result;
	}
	...
	public final void dump() {
		if (parent != null) parent.dump();
		if (fQualifiedForEndLevelBonus) System.out.print("BONUS ");
		System.out.println("2F:"+getFScore()+" = G:"+getGScore()+" + H:"+getHScore());
		f.dump();
		System.out.println("===============================================");
	}
	...
	public static class Factory extends Option.Factory {
		...
		public Option create(Frame f, int nCycleCnt) {
			return new MoveRightBumpBricksOption(f, nCycleCnt);
		}

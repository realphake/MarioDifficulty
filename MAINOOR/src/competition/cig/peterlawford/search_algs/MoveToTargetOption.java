...
import competition.cig.peterlawford.simulator.TheoreticEnemy;
...
public class MoveToTargetOption extends Option<MoveToTargetOption> {
...
//	}
	...
	public MoveToTargetOption(MoveToTargetOption old, Frame f, 
			int coded_action, BlockEvaluator block_eval) {
		super(old, f, coded_action);
		pt = findTarget(f.enemies);
...
//		hScore = -calc_dist;
	...
	public MoveToTargetOption(Frame f, int nCycleCnt) {
		super(null, f, -1);
		pt = findTarget(f.enemies);
//		if (pt == null) throw new java.lang.NullPointerException();
...
//	}
	...
	public MoveToTargetOption clone_and_guess(int i,
			BlockEvaluator block_eval) {
		Frame f_new = new Frame(f, false);
		return new MoveToTargetOption(this, f_new, i, block_eval);		
	}

	private Point2D.Float findTarget(TheoreticEnemies enemies) {
//		Point2D.Float pt = null;
		for (TheoreticEnemy e : enemies.enemies) {
			if (e.nType == 14) {
				return new Point2D.Float(e.x, e.y);
//				return pt;
			}
		}
//		throw new java.lang.NullPointerException();
...
//		public void setPoint(Point2D.Float pt) { this.pt = pt; }
		...
		public Option create(Frame f, int nCycleCnt) {
			return new MoveToTargetOption(f, nCycleCnt);
		}

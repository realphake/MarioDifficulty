    ...
    private PlanRunner planJump(Edge currentFloor, Edge targetFloor) {
        ...
        for (jumpTime = 0; jumpTime <= 7; jumpTime++) {
            ...
            if (ticks <= flightTime) {
                ...
                plan.addKey(Keys.SPEED);
                plan.addKey(Keys.JUMP, 0, jumpTime);
                plan.addKey(Keys.RIGHT, flightTime - ticks, ticks);
            } else {
                ...
                plan.addKey(Keys.SPEED);
                plan.addKey(Keys.RIGHT, 0, ticks);
                plan.addKey(Keys.JUMP, timeUntilJump, jumpTime);
            }
        ...
        //System.out.println(String.format(" * Can't compute a flight time for jump to %d blocks",
        //        blocks));
        return null;
    }
...
//        @Override
    ...
    public PlanRunner planMovement() {
        ...
        if (currentFloor != null) {
            //System.out.println(String.format("Found current floor: (%f,%f)-(%f,%f)",
            //        currentFloor.x1, currentFloor.y1, currentFloor.x2, currentFloor.y2));
        } else {
            //System.out.println("Not standing on anything!  TODO: need to track fall and make sure Mario lands on something!");
            return null;
        }
        ...
        // No target floor; try running to the right on current floor.
        /*if (mario.x < currentFloor.x2) {
            //System.out.println("Nothing to jump to, just run right");
            PlanRunner plan = new PlanRunner();
            plan.addKey(Keys.SPEED);
            plan.addKey(Keys.RIGHT, 0, 1);
            return plan;
        }*/

        //System.out.println("Can't find a target floor!  TODO: now what?");
        return null;
    }

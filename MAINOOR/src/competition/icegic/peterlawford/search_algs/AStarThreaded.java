...
class ProcessingThread extends Thread {
	...
	LinkedList<SlowAgent.DisplayPathInfo> lines = null;
	...
	public static void addFutureOptions(PriorityBlockingQueue<Option> q,
			Option x, LinkedList<SlowAgent.DisplayPathInfo> lines) {
		int nCnt = 0;
		for (int i=0; i<AStarThreaded.ENCODED_CHOICEVAL_CNT; i++) {	
			...
			Option y = x.clone_and_guess(i, lines);
			nCnt++;
			q.add(y);
		}
	}
...
//						e.printStackTrace();
				...
				addFutureOptions(open_set, x, lines);
...
public class AStarThreaded extends AbstractSearch {
	...
	static final int ENCODED_CHOICEVAL_CNT = 10;
	...
	private void restart(Frame f0) {
		if (blocking_grid != null) {
			blocking_grid.clear();
		}

		open_set.clear();				
		Option start = new Option(f0, initial_cycle_cnt);
		if (FLAG_INVERTED) {
			ProcessingThread.addFutureOptions(open_set, start, null);
		} else {
			open_set.add(start);
		}		
	...
	public AStarThreaded() {
		...
		for (int i=0; i<nrOfProcessors; i++) {
			ProcessingThread thread = new ProcessingThread(
					open_set, blocking_grid, goal_eval, Thread.currentThread());
			threads.add(thread);
			thread.start();
		}
...
*/
		 ...
		 if (best_result_t == null) best_result_t = open_set.peek();
		 ...
		 if (!open_set.isEmpty()) {
			 ...
			 int result = prev_best_first_choicepoint.reconstruct_path();

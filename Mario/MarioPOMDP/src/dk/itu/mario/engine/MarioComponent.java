...
public class MarioComponent extends JComponent implements Runnable, KeyListener, FocusListener, MouseListener {
    ...
    private void resetKeys() {
        scene.toggleKey(Mario.KEY_LEFT, false);
        scene.toggleKey(Mario.KEY_RIGHT, false);
        scene.toggleKey(Mario.KEY_DOWN, false);
        scene.toggleKey(Mario.KEY_UP, false);
        scene.toggleKey(Mario.KEY_SPEED, false);
        scene.toggleKey(Mario.KEY_JUMP, false);
        scene.toggleKey(Mario.KEY_ENTER, false);
    }
    ...
    public void start() {
        if (!running) {
            running = true;
            th = new Thread(this, "Game Thread");
            th.start();
        }
    }
    ...
    public void pause() {
        System.out.println("");
        System.out.println("Recording game experience observations...");
        //System.out.println("Pausing and reseting key presses...");
        resetKeys();
		paused = true;
    }
    ...
    public void run() {
        ...
        while (running) {
            if (paused) {
                ...
                if (wasPaused){
                    totalPauseTime += (System.nanoTime() - startTime - totalPauseTime) - lastFrameRenderTime;
                    frameRenderTime = lastFrameRenderTime;
                    wasPaused = false;
                } else {
                    frameRenderTime = (System.nanoTime() - startTime - totalPauseTime);
                }
                time = frameRenderTime / 1000000000f;
    ...
    public void toRandomGame() {
        //LevelSceneTest(GraphicsConfiguration graphicsConfiguration, MarioComponent renderer, long seed, int levelDifficulty, int type, boolean isCustom)
